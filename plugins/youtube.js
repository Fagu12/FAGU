...
Function({
	...
	if (isUrl(match) && match.includes('youtu')) {
		...
		let writer = await addAudioMetaData(await getBuffer(media.dl_link), thumb, media.title, `${config.BOT_INFO.split(";")[0]}`, 'Fagu Official')
...
Function({
	...
	if (isUrl(match) && match.includes('youtu')) {
		...
		const file = await addAudioMetaData(await getBuffer(result.dl_link), thumbnail, result.title, `${config.BOT_INFO.split(";")[0]}`, 'Fagu Official')
